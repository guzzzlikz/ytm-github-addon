...
public class Main {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        while(true) {
            ...
            try (AsyncHttpClient client = Dsl.asyncHttpClient()) {
                song = client.prepareGet(ytmUrl)
                        .addHeader("Authorization", "Bearer " + ytmToken)
                        .execute()
                        .toCompletableFuture()
                        .thenApply(response -> {
                            try {
                                ...
                                System.out.println("YTMD request failed!");
                                return null;
                            }
                        })
                        .join();
    ...
    public static Config verifyData(Config c) throws IOException {
        String ytmToken = c.getYtmToken();
        String gitToken = c.getGitToken();
        int port = c.getPort();
        String url = c.getUrl();
        ...
        while (choice != 0) {
            ...
            switch (choice) {
                ...
                case 2 -> {
                    System.out.println("old YoutubeMusicDesktop token: " + ytmToken);
                    System.out.print("new YoutubeMusicDesktop token: ");
                    ytmToken = scanner.nextLine();
                    c.setYtmToken(ytmToken);
                    objectMapper.writeValue(new File("config.json"), c);
                }
                case 3 -> {
                    System.out.println("old GitHub token: " + gitToken);
